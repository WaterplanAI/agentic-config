...
```
${CLAUDE_PLUGIN_ROOT}/scripts/prompts/
...
those. Command and skill definitions stay in plugin-local folders.
The `scripts/prompts/` tree is consumed by `${CLAUDE_PLUGIN_ROOT}/scripts/tools/` tools.
...
```
${CLAUDE_PLUGIN_ROOT}/scripts/tools/
  spawn.py                          # Layer 0: Primitive (singleton)
  <name>.py                         # Layer 1: Executor
  o<name>.py                        # Layer 2: Orchestrator
  coordinator.py                    # Layer 3: Coordinator
  campaign.py                       # Layer 4: Campaign
```
...
| Layer | Location |
|-------|----------|
| Executor (command) | `commands/<name>.md` |
| Executor (skill) | `skills/<name>/SKILL.md` |
| Executor (agent) | `agents/<name>.md` |
| Orchestrator+ | `${CLAUDE_PLUGIN_ROOT}/scripts/prompts/<layer>/<name>.md` |

### Config Files (Layer 2+)

```
${CLAUDE_PLUGIN_ROOT}/scripts/tools/config/
...
```json
{
  "stages": [
    {
      "name": "research",
      "executor": "scripts/tools/spec.py",
      "args": ["RESEARCH"],
      "model": "medium-tier",
      "retry": 1,
      "required": true
...
|-------|---------|---------------|--------|
| **0** | Primitive | `plugins/ac-workflow/scripts/tools/spawn.py` | Done |
| **1** | Executor (spec) | `plugins/ac-workflow/scripts/tools/spec.py` | Done |
| **1** | Executor (researcher) | `plugins/ac-workflow/scripts/tools/researcher.py` | Done |
| **2** | Orchestrator (ospec) | `plugins/ac-workflow/scripts/tools/ospec.py` | Done |
| **2** | Orchestrator (oresearch) | `plugins/ac-workflow/scripts/tools/oresearch.py` | Done |
| **3** | Coordinator | `plugins/ac-workflow/scripts/tools/coordinator.py` | Done |
| **4** | Campaign | `plugins/ac-workflow/scripts/tools/campaign.py` | Done |

### Supporting Infrastructure

| Component | Implementation | Status |
|-----------|---------------|--------|
| Shared library | `plugins/ac-workflow/scripts/tools/lib/__init__.py` | Done |
| Observability module | `plugins/ac-workflow/scripts/tools/lib/observability.py` | Done |
| Stage config (ospec) | `plugins/ac-workflow/scripts/tools/config/ospec.json` | Done |
| Research config | `plugins/ac-workflow/scripts/tools/config/oresearch.json` | Done |
| Phase config (example) | `plugins/ac-workflow/scripts/tools/config/coordinator-example.json` | Done |
| Executor prompts | `plugins/ac-workflow/scripts/prompts/executors/researcher-{market,ux,tech,consolidator}.md` | Done |
| Orchestrator prompt | `plugins/ac-workflow/scripts/prompts/orchestrators/ospec.md` | Done |
| Coordinator prompt | `plugins/ac-workflow/scripts/prompts/coordinators/coordinator.md` | Done |
| Campaign prompts | `plugins/ac-workflow/scripts/prompts/campaigns/{campaign,refinement-evaluator,roadmap-writer,evaluator}.md` | Done |
| Signal protocol | `plugins/ac-workflow/skills/mux/tools/signal.py` | Pre-existing |
| Signal polling | `plugins/ac-workflow/skills/mux/tools/poll-signals.py` | Pre-existing |
| Session lifecycle | `plugins/ac-workflow/skills/mux/tools/session.py` | Pre-existing |
