...
| `uv run .claude/skills/mux/tools/check-signals.py` | One-shot signal check (fallback) |
| `uv run .claude/skills/mux/tools/extract-summary.py` | Bounded report access (TOC + Executive Summary) |
...
Task(prompt="Read .claude/skills/mux/agents/sentinel.md. Check git status.", model="sonnet", run_in_background=True)

# Pattern search -> auditor
Task(prompt="Read .claude/skills/mux/agents/auditor.md. Search for pattern.", model="sonnet", run_in_background=True)
```
