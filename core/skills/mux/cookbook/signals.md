...
```bash
uv run .claude/skills/mux/tools/signal.py "$SIGNAL_PATH" --path "$OUTPUT_PATH" --status success
```

Example:
```bash
uv run .claude/skills/mux/tools/signal.py \
...
```bash
uv run .claude/skills/mux/tools/verify.py "$SESSION_DIR" --action missing --expected 5
```
...
```bash
uv run .claude/skills/mux/tools/signal.py "$SIGNAL_PATH" --path "$OUTPUT_PATH" --status error
```
