...
1. POLL SIGNALS
   ...
   result=$(uv run .claude/skills/mux/tools/poll-signals.py "$session_dir" --expected $expected --timeout 300)
...
result=$(uv run .claude/skills/mux/tools/poll-signals.py "$session_dir" --expected $expected --timeout $timeout)
...
**Rule**: Orchestrator should verify signals via `uv run .claude/skills/mux/tools/verify.py`, not wait for Task notifications.
...
1. Poll signals (single blocking call):
   result=$(uv run .claude/skills/mux/tools/poll-signals.py "{session_dir}" --expected {N} --timeout 300)
