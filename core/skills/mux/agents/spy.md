...
6. CREATE SIGNAL
   uv run .claude/skills/mux/tools/signal.py "{signal_path}" --path "{report_path}" --status success
...
2. Create signal via: `uv run .claude/skills/mux/tools/signal.py {SIGNAL} --path {REPORT} --status success`
3. Return exactly: `0`
...
```
Read .claude/skills/mux/agents/spy.md for full protocol.
