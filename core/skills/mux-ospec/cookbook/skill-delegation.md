...
# Orchestrator context remains clean, only monitors signal
Task(
...
## Worker + Monitor Pattern

Every stage delegation requires a monitor in the SAME message:
...
Task(
    prompt=f"""Read agents/monitor.md. EXPECTED: 1. Use poll-signals.py to track.

SESSION: tmp/mux/20260206-1430-auth
EXPECTED_SIGNALS: [".signals/phase-1-plan.done"]
...
# Checkpoint
# PLAN worker launched (opus, background)
...
**Impact:** Orchestrator blocked. Cannot launch monitor. Cannot launch next stage. Violates MUX non-blocking principle.

### Violation 6: Missing Monitor

```python
# FATAL: Worker launched without monitor
Task(prompt="Invoke Skill(skill='spec', args='PLAN ...')", model="opus", run_in_background=True)

# No monitor launched in same message
# Orchestrator has no visibility into completion
```

**Impact:** Orchestrator does not know when stage completes. Must manually check signals. Defeats MUX automation.

### Violation 7: Polling Agent Output
...
# 3. Orchestrator launches PLAN stage (worker + monitor, same message)
Task(
    prompt="""Invoke Skill(skill="spec", args="PLAN specs/2026/02/auth/001-system.md ultrathink").
...
Task(
    prompt="""Read agents/monitor.md. EXPECTED: 1. Use poll-signals.py.
...
- Monitor tracking signal: .signals/phase-1-plan.done
...
# 7. Monitor detects signal, notifies orchestrator
...
3. Launch monitors in SAME message as workers
