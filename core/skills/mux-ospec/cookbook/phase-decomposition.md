...
### File Overlap Constraint (HARD RULE)

**Before launching ANY wave with >1 phase, check for target file overlap.**

Agents in the same wave that EDIT the same source file will clobber each other's writes. There is no merge/reconciliation strategy — the second write destroys the first.

```python
def validate_wave_safety(wave_phases):
    """Block waves with file overlap."""
    file_map = {}  # file -> phase_num
    for phase in wave_phases:
        for target_file in phase.get("target_files", []):
            if target_file in file_map:
                raise FileConflictError(
                    f"File {target_file} targeted by both "
                    f"phase {file_map[target_file]} and phase {phase['num']}. "
                    f"Split into sequential waves."
                )
            file_map[target_file] = phase["num"]
```

If overlap detected: split conflicting phases into separate sequential waves.
...
FINAL: Return EXACTLY: done""",
    ...
    # Wave completion: runtime task-notifications arrive as workers complete
    # After all N notifications received, run verify.py as safety check
```
