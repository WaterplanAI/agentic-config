...
Monitor signals for expected completion.
...
## Monitor Timeout Recovery

### Never Self-Monitor

If monitor times out, launch NEW monitor - never poll directly.

```python
# WRONG - context suicide
while not signals_complete():
    time.sleep(5)  # NEVER DO THIS

# RIGHT - launch new monitor
Task(
    prompt=f"""Read agents/monitor.md.

SESSION: {{session}}
EXPECTED: {remaining_count}
STARTING_OFFSET: {completed_count}
