---
name: ac-workflow-product-manager
...
When generating `o_spec_config` for each phase, use this mapping as downstream execution metadata:
...
Bundling reduces orchestration overhead by combining related phases into single downstream spec-execution cycles.
...
phases:
  - id: "phase-1"
    ...
    spec_prompt: |
      Inline prompt for downstream spec execution.
...
Invoke when:
- Feature requires multiple structured spec-execution cycles
- Scope is unclear and needs breakdown
- Dependencies between parts are complex
- Parallel development is desired

Output is consumed by downstream spec execution workflows.
