...
# SC-5: All 5 plugins load simultaneously without namespace conflicts
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "$0")/.." && pwd)"
PLUGINS=(ac-workflow ac-git ac-qa ac-tools ac-meta)

echo "=== Load all 5 plugins simultaneously ==="
PLUGIN_DIRS=""
for p in "${PLUGINS[@]}"; do
...
echo "1. /help -- all skills from all 5 plugins visible"
echo "2. Core skills resolve directly: /spec, /pull-request, /gsuite, /skill-writer"
...
echo "1. Install all 5 via marketplace"
...
echo "[ ] SC-5: Core skills resolve directly (/spec, /pull-request, /gsuite, /skill-writer)"
