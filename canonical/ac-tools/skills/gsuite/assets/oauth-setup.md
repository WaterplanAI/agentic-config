...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/setup.py init --project YOUR_PROJECT_ID

# Without gcloud (prints direct console URLs)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/setup.py init
```
...
```bash
# Check setup status
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/setup.py check

# Or view account status
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/auth.py status
```
