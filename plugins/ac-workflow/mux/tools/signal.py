...
# ///
"""
Signal file creation tool for mux agents.
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Create signal file for mux agent completion"
    )
    parser.add_argument(
        "signal_path",
    ...
    # Detect skill-relative pollution (paths resolved under skill directories instead of project root)
    signal_path_str = str(signal_path).replace("\\", "/")
    if re.search(r"(?:^|/)(?:\.claude/)?skills/(?:mux|mux-subagent)/tmp/mux/", signal_path_str):
        raise ValueError(
            f"Signal path appears skill-relative: {signal_path}\n"
            "Use a project-root-relative path (e.g., tmp/mux/<session>/.signals/<name>.done)."
        )
