...
**Context Access**: Use `uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/extract-summary.py <file> --max-bytes 1024` for bounded reads.
...
1. Use `uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/extract-summary.py <file> --max-bytes 1024` for bounded context
...
```
Read ${CLAUDE_PLUGIN_ROOT}/skills/mux/agents/coordinator.md for full protocol.
