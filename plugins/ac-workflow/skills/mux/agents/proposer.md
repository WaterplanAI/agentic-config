...
3. GENERATE NEXT ACTION PROMPT
   ...
   PASS_ITERATE (minor improvements suggested):
     ...
     Format: /mux lean - {specific enhancement with paths}
     Example: /mux lean - Enhance section 3.2 of {deliverable_path} with {specific improvement}

   FAIL_GAPS (missing coverage):
     Suggest gap-filling research/audit prompt
     Format: /mux {research focus}. Session: {session_dir}. Fill gaps in {sections}.
     Example: /mux Research competitor patterns focusing on X. Session: {session_dir}. Fill gaps in sections 2.1 and 4.3.

   FAIL_QUALITY (quality issues):
     Suggest rewrite prompt with specific fixes
     Format: /mux lean - Rewrite {deliverable_path} section {N} to {specific fixes}
     Example: /mux lean - Rewrite {deliverable_path} section 5 to include concrete acceptance criteria and remove vague language

   PARTIAL (agent failures):
     Suggest retry prompt for failed components
     Format: /mux Retry failed agents: {agent_ids}. Session: {session_dir}. Previous errors: {error_summary}.
     Example: /mux Retry failed research agents: 003-security, 005-compliance. Session: {session_dir}. Previous errors: context timeout.
...
- MUX continuation: `/mux lean - fix X`, `/mux research Y`
- Git operations: `/commit`, `/pull-request`, `/git-safe`
- Spec workflow: `/spec IMPLEMENT`, `/spec REVIEW`
...
```
/mux lean - Enhance section 3.2 of $PROJECT_ROOT/tmp/mux/20260130-0953-auth/deliverable/roadmap.md with specific migration timeline estimates based on team velocity
```

### FAIL_GAPS (missing coverage)

```
/mux Research competitor authentication patterns (Auth0, Okta, Cognito) focusing on enterprise SSO integration. Session: $PROJECT_ROOT/tmp/mux/20260130-0953-auth. Fill gaps in sections 2.1 and 4.3.
```

### FAIL_QUALITY (quality issues)

```
/mux lean - Rewrite $PROJECT_ROOT/tmp/mux/20260130-0953-auth/deliverable/roadmap.md section 5 to include concrete acceptance criteria and remove vague language ("might", "could", "possibly")
```

### PARTIAL (agent failures)

```
/mux Retry failed research agents: 003-security-patterns, 005-compliance-requirements. Session: $PROJECT_ROOT/tmp/mux/20260130-0953-auth. Previous errors: context timeout.
```
...
- Start with command: `/mux`, `/commit`, `/pull-request`, etc.
- Include ALL required context inline
- Use absolute paths, not relative
- Specify session dir if continuing mux
...
SESSION DIR: $PROJECT_ROOT/tmp/mux/20260130-0953-auth
ORIGINAL TASK: Research OAuth2 implementation best practices
SIGNAL: $PROJECT_ROOT/tmp/mux/20260130-0953-auth/.signals/proposer.done
