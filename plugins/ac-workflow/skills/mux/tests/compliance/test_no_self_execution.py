...
- Task (delegation)
- Bash (only: mkdir, uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/*.py)
...
ALLOWED_TOOLS = [
    "Task",
    "Bash",  # Only for mkdir-p and uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/*.py
    "AskUserQuestion",
...
def test_bash_only_allowed_patterns(inspector):
    ...
    # Allowed: uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/*.py
    inspector.record("Bash", {"command": "uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/verify.py /session --action summary"})
    ...
    with pytest.raises(AssertionError, match="Forbidden Bash"):
        assert len(forbidden_bash) == 0, (
            ...
            "Orchestrator Bash is only for: mkdir -p, uv run ${CLAUDE_PLUGIN_ROOT}/skills/mux/tools/*.py"
        )
