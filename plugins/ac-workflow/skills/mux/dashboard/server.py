...
_metrics_module = import_module("metrics")
collect_session_metrics = _metrics_module.collect_session_metrics
export_prometheus = _metrics_module.export_prometheus

app = FastAPI(title="MUX Observability Dashboard", version="1.0.0")

# Configuration - set via CLI args or defaults
SESSIONS_DIR = Path("tmp/mux")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="MUX observability dashboard server")
    parser.add_argument("--host", default="0.0.0.0", help="Bind host")
    parser.add_argument("--port", type=int, default=8080, help="Bind port")
    parser.add_argument(
        "--sessions-dir",
        type=Path,
        default=Path("tmp/mux"),
