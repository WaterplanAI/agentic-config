...
from lib import EXIT_FAILURE, EXIT_INTERRUPTED, EXIT_SUCCESS, resolve_project_file
...
SPEC_COMMAND_RELATIVE = Path("skills", "spec", "SKILL.md")
...
def resolve_spec_command(script_dir: Path) -> Path:
    """Resolve the spec SKILL.md path via shared project resolver.

    Raises:
        FileNotFoundError: If SKILL.md cannot be found.
    """
    try:
        return resolve_project_file(SPEC_COMMAND_RELATIVE, script_dir)
    except FileNotFoundError as exc:
        raise FileNotFoundError(
            f"Cannot find {SPEC_COMMAND_RELATIVE}. "
            "Run from within the ac-workflow plugin tree."
        ) from exc


def resolve_spawn_script(script_dir: Path) -> Path:
    """Resolve the spawn.py script path via shared project resolver.

    Raises:
        FileNotFoundError: If spawn.py cannot be found.
    """
    try:
        return resolve_project_file(SPAWN_SCRIPT_RELATIVE, script_dir)
    except FileNotFoundError as exc:
        raise FileNotFoundError(
