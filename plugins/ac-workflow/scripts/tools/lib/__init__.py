...
SPAWN_SCRIPT_RELATIVE = Path("scripts", "tools", "spawn.py")
CONFIG_DIR_RELATIVE = Path("scripts", "tools", "config")
TEMPLATES_DIR_RELATIVE = CONFIG_DIR_RELATIVE / "templates"
...
def resolve_project_file(relative_path: Path, script_dir: Path | None = None) -> Path:
    """Resolve a plugin-root-relative file path.
    ...
    raise FileNotFoundError(
        f"Cannot find {relative_path}. Run from within the plugin tree."
    )
...
def get_project_root(script_dir: Path | None = None) -> Path:
    ...
    Raises:
        FileNotFoundError: If plugin root cannot be found.
    """
    marker = SPAWN_SCRIPT_RELATIVE
    ...
    raise FileNotFoundError(
        "Cannot find plugin root. Run from within the plugin tree."
    )


def resolve_config_path(value: str, project_root: Path) -> Path:
    """Resolve path values from config files.

    Supports:
    - plugin-root-relative paths (`scripts/tools/spec.py`)
    - `${CLAUDE_PLUGIN_ROOT}/...` placeholders
    - absolute paths
    """
    if value.startswith("${CLAUDE_PLUGIN_ROOT}/"):
        value = value.removeprefix("${CLAUDE_PLUGIN_ROOT}/")
...
def load_stage_config(config_name: str, script_dir: Path | None = None) -> dict:
    ...
    """
    config_relative = CONFIG_DIR_RELATIVE / f"{config_name}.json"
...
def load_template(name_or_path: str, script_dir: Path | None = None) -> dict:
    ...
    if name_or_path.endswith(".json"):
        ...
        template_relative = TEMPLATES_DIR_RELATIVE / f"{name_or_path}.json"
