...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py comments <file_id> [--json] [--author NAME] [--since YYYY-MM-DD] [--include-deleted] [--suggestions] [--limit N] [--account EMAIL]

# Reply to a comment (auto-prefixes "@ac-reply: ")
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply <file_id> <comment_id> "<content>" [--resolve] [--yes] [--account EMAIL] [--json]

# Resolve a comment without replying
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py resolve <file_id> <comment_id> [--yes] [--account EMAIL] [--json]
```
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py find <doc_id> "<query>" [--tab TAB] [--context N] [--account EMAIL] [--json]

# Replace text (single edit)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py edit <doc_id> --find "<old>" --replace "<new>" [--tab TAB] [--yes] [--account EMAIL] [--json]

# Replace text (batch from plan file)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py edit <doc_id> --plan /tmp/edit-plan.json [--tab TAB] [--yes] [--account EMAIL] [--json]

# Read document content
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py read <doc_id> [--tab TAB] [--json] [--raw] [--account EMAIL]
```
...
```bash
# Read range
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py read <spreadsheet_id> "<range>" [--json] [--account EMAIL]

# Write value(s)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py write <spreadsheet_id> "<range>" "<value>" [--account EMAIL] [--json]
```
...
```bash
# Read presentation
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py read <presentation_id> [--json] [--raw] [--account EMAIL]

# Replace text (single edit)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py edit <presentation_id> --find "<old>" --replace "<new>" [--yes] [--account EMAIL] [--json]

# Replace text (batch from plan file)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py edit <presentation_id> --plan /tmp/edit-plan.json [--yes] [--account EMAIL] [--json]
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py comments <file_id> --json
```
...
1. Find the quoted text location:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py find <doc_id> "<quotedFileContent.value>" --json
   ```
...
3. Read full document if broader context is needed:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py read <doc_id> --json
   ```
...
1. Read the relevant sheet range:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py read <spreadsheet_id> "<range>" --json
   ```
...
1. Read the presentation:
   ```bash
   uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py read <presentation_id> --json
   ```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py edit <doc_id> --plan /tmp/edit-plan.json --yes
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py write <spreadsheet_id> "<range>" "<value>"
```

#### Presentations

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py edit <presentation_id> --plan /tmp/edit-plan.json --yes
```
...
**Edit applied:**
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply <file_id> <comment_id> "Applied: replaced '<old_snippet>' with '<new_snippet>'" --resolve --yes
```

**Noop (skipped):**
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply <file_id> <comment_id> "Skipped: <reason>" --yes
```
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py write <id> "Sheet1!B3" "new value"
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py write <id> "Sheet1!C5" '["val1", "val2"]'
```
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py comments abc123 --json
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py find abc123 "quarterly report" --json
...
# Phase 3: Apply
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/docs.py edit abc123 --plan /tmp/edit-plan.json --yes

# Phase 4: Reply + resolve
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply abc123 AAAA1234 "Applied: replaced 'quarterly report' with 'Q4 2025 report'" --resolve --yes
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply abc123 BBBB5678 "Skipped: question, not an edit instruction" --yes
```
...
```bash
# Phase 1: Fetch comments
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py comments def456 --json
# Comment id="CCCC9012", content="@ac change this to 500", quotedFileContent.value="350"

# Phase 2: Read sheet, identify cell
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py read def456 "Sheet1!A1:Z100" --json
# Locate cell containing "350" -> Sheet1!D7

# Phase 3: Apply
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/sheets.py write def456 "Sheet1!D7" "500"

# Phase 4: Reply + resolve
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply def456 CCCC9012 "Applied: changed D7 from '350' to '500'" --resolve --yes
```
...
```bash
# Phase 1: Fetch comments
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py comments ghi789 --json
# Comment id="DDDD3456", content="@ac change 'Draft' to 'Final' on this slide"

# Phase 2: Read presentation, identify page
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py read ghi789 --json
...
# Phase 3: Apply
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/slides.py edit ghi789 --plan /tmp/edit-plan.json --yes

# Phase 4: Reply + resolve
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/drive.py reply ghi789 DDDD3456 "Applied: replaced 'Draft' with 'Final' on slide g1234abc" --resolve --yes
```
