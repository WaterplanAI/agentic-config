...
active=$(uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/auth.py status --json | jq -r '.active')
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py list-events --days 7

# Create event (uses local timezone by default)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py create "Meeting" "2024-01-15T10:00" "2024-01-15T11:00" --yes

# Create event with explicit timezone
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py create "Meeting" "2024-01-15T10:00" "2024-01-15T11:00" \
  --timezone "America/New_York" --yes

# Create event with attendees and Meet link (include self)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py create "Team Sync" "2024-01-15T10:00" "2024-01-15T11:00" \
  --attendees "self@example.com,colleague@example.com" --meet --yes

# Create private event (hidden from others)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py create "Personal" "2024-01-15T10:00" "2024-01-15T11:00" \
  --visibility private --yes

# Create event that shows as free (doesn't block calendar)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py create "Optional" "2024-01-15T10:00" "2024-01-15T11:00" \
  --show-as free --yes

# Update event (preserves original timezone unless --timezone specified)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py update <event_id> --title "New Title"

# Update event time with explicit timezone
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py update <event_id> --start "2024-01-15T11:00" \
  --timezone "America/New_York" --yes

# Add/remove attendees
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py update <event_id> --add-attendees "new@example.com" --yes
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py update <event_id> --remove-attendees "old@example.com" --yes

# Update visibility and show-as
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py update <event_id> --visibility private --show-as free --yes

# Delete event (requires confirmation)
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py delete <event_id>

# List calendars
uv run ${CLAUDE_PLUGIN_ROOT}/skills/gsuite/tools/gcalendar.py calendars
```
