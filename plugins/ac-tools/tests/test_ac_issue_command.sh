...
test_issue_command_exists() {
  ...
  local cmd_file="$REPO_ROOT/plugins/ac-tools/skills/ac-issue/SKILL.md"

  assert_file_exists "$cmd_file" "ac-issue SKILL.md exists in ac-tools plugin"
}
...
test_issue_frontmatter_valid() {
  ...
  assert_file_contains "$cmd_file" "name: ac-issue" "Has ac-issue name field"
...
test_issue_target_repo() {
  ...
  assert_file_contains "$cmd_file" "WaterplanAI/agentic-config" "Targets correct repository"
...
test_issue_environment_collection() {
  ...
  assert_file_contains "$cmd_file" "CLAUDE_PLUGIN_ROOT" "Uses plugin-root path resolution"
}
